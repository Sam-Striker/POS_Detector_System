...
def results(request):
    model = joblib.load(os.path.join(os.path.dirname(__file__),'PCOS_Pred.joblib'))
    ...
    result_msg = ''
    if ans == 0:
        result_msg = 'You do not have Polycystic ovary syndrome (PCOS).'
    else:
       result_msg = 'You have Polycystic ovary syndrome (PCOS) .'
